/* clientext extension for PHP */

#ifndef PHP_CLIENTEXT_H
# define PHP_CLIENTEXT_H

extern zend_module_entry clientext_module_entry;
# define phpext_clientext_ptr &clientext_module_entry

# define PHP_CLIENTEXT_VERSION "0.1.0"

#define CALLBACK_HASH_SIZE 512

#define NMID_LOGO_BASE64 "data:image/png;base64,"

#define SUPPORT_URL "http://www.niansong.top"

#define LOGO_IMG "<a href=" SUPPORT_URL "> \
				          <img src=\"" NMID_LOGO_BASE64 "\" alt=\"nmid logo\" /></a>\n"

# if defined(ZTS) && defined(COMPILE_DL_CLIENTEXT)
ZEND_TSRMLS_CACHE_EXTERN()
# endif

#ifndef ZEND_THIS
#define ZEND_THIS (&EX(This))
#endif

ZEND_BEGIN_MODULE_GLOBALS(clientext)
    struct _sHashTable *callbacks;
ZEND_END_MODULE_GLOBALS(clientext)

PHP_MINIT_FUNCTION(clientext);
PHP_MSHUTDOWN_FUNCTION(clientext);
PHP_RINIT_FUNCTION(clientext);
PHP_RSHUTDOWN_FUNCTION(clientext);
PHP_MINFO_FUNCTION(clientext);

#ifdef ZTS
#include "TSRM.h"
#define CLIENTEXT_G(v) TSRMG(clientext_globals_id, zend_clientext_globals*, v)
extern int clientext_globals_id;
#else
#define CLIENTEXT_G(v) (clientext_globals.v)
extern zend_clientext_globals clientext_globals;
#endif

static
zval* n_zend_read_property(zend_class_entry *ce, zval *obj, const char *s, int len, int silent)
{
    zval rv, *property = zend_read_property(ce, obj, s, len, silent, &rv);
    if(UNEXPECTED(property == &EG(uninitialized_zval)))
    {
        zend_update_property_null(ce, obj, s, len);
        return zend_read_property(ce, obj, s, len, silent, &rv);
    }

    return property;
}

#endif	/* PHP_CLIENTEXT_H */

