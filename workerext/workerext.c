...
typedef struct _nmid_worker_cb nmid_worker_cb;
struct _nmid_worker_cb {
    char *fname;
...
doCallBack(Response *Res)
{
    ...
    nmid_worker_cb *worker_cb;
    hash_lookup(WORKEREXT_G(callbacks), fname, (void **)&worker_cb);
    ...
    if(UNEXPECTED(n_zend_call_function_ex2(NULL, &worker_cb->fci_cache, 1, call_params, &retval) != SUCCESS)) {
...
PHP_METHOD(workerext, add_function)
{
    ...
    if(fci.size) {
        n_zend_fci_cache_persist(&fci_cache);
    }
    ...
    nmid_worker_cb *worker_cb;
    worker_cb = emalloc(sizeof(nmid_worker_cb));
    memset(worker_cb, 0, sizeof(nmid_worker_cb));

    worker_cb->fname = fname;
    worker_cb->fci_cache = fci_cache;
    hash_insert(WORKEREXT_G(callbacks), fname, worker_cb);
}
...
PHP_METHOD(workerext, del_function)
{
    ...
    hash_remove(WORKEREXT_G(callbacks), fname, (void **)&worker_cb);
